...
def draw_tm_tape(screen, tape_name: str, tape: alang.Tape, origin=(100, 100)):
    ...
    if tape_name not in tapes_names_rendered:
        tapes_names_rendered[tape_name] = \
            MAIN_FONT.render(f"Tape: `{tape_name}`", True, TAPE_NAME_COLOR)
    ...
    for index in range(min_index, max_index + 1):
        ...
        tape_square_rect = [origin[0] +
                            index * (TAPE_SQUARE_SIZE + BETWEEN_SQUARES_SPACE)
                            - CAMERA_POS[0], origin[1] - CAMERA_POS[1],
                            TAPE_SQUARE_SIZE, TAPE_SQUARE_SIZE]

        if index in tape.tape:
            if tape.tape[index]:
                symbol = tape.tape[index]
                if symbol not in alphabet_rendered:
                    alphabet_rendered[symbol] = MAIN_FONT_BIGGER.render(
                        f"{symbol}", True, ALPHABET_COLOR)

                symbol_pos = tape_square_rect[:2]
                symbol_pos[0] += tape_square_rect[2]/2 - \
                    alphabet_rendered[symbol].get_size()[0]/2
                symbol_pos[1] += tape_square_rect[3] / \
                    2 - alphabet_rendered[symbol].get_size()[1]/2

                screen.blit(
                    alphabet_rendered[symbol], symbol_pos)
